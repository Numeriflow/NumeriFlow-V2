...
11. **migrateCheck():** Runs on dashboard load — if `nf_user` exists but no `nf_sb_session`, clears localStorage and redirects to login. Catches pre-Supabase legacy accounts.
12. **_signingUp guard:** Boolean in signup.html prevents double-submit and race conditions — set to `true` on first click, released on success, error, or early return.
13. **saveFeedbackToSupabase():** Beta feedback saves to Supabase `feedback` table + Formspree + email simultaneously. Requires `feedback` table to exist (see SQL above).
